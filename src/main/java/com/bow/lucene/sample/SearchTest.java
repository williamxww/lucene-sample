...
public class SearchTest {
	...
	/**
	 * 对特定项搜索 按词条搜索—TermQuery TermQuery是最简单、也是最常用的Query。TermQuery可以理解成为“词条搜索”，
	...
	/**
	 * “多条件查询”搜索—BooleanQuery BooleanQuery也是实际开发过程中经常使用的一种Query。
	...
	/**
	 * TermRangeQuery 范围查询 TermRangeQuery是用于字符串范围查询的，既然涉及到范围必然需要字符串比较大小，
	 * 字符串比较大小其实比较的是ASC码值，即ASC码范围查询。 一般对于英文来说，进行ASC码范围查询还有那么一点意义，
	...
	public void testTermRangeQuery() throws Exception {
		...
		/**
		 * field 字段 lowerterm -范围的下端的文字 upperterm -范围的上限内的文本 includelower
		 * -如果真的lowerterm被纳入范围。 includeupper -如果真的upperterm被纳入范围。
	...
	/**
	 * 所谓PhraseQuery，就是通过短语来检索，比如我想查“big car”这个短语， 那么如果待匹配的document的指定项里包含了
	 * "big car"这个短语， 这个document就算匹配成功。可如果待匹配的句子里包含的是“big black car”，
	...
	/**
	 * 相近词语的搜索—FuzzyQuery FuzzyQuery是一种模糊查询，它可以简单地识别两个相近的词语。
	 * 
	 * @throws Exception
	 */
	@Test
	...
	/**
	 * 使用通配符搜索—WildcardQuery Lucene也提供了通配符的查询，这就是WildcardQuery。
	...
	/**
	 * 解析查询表达式 QueryParser实际上就是一个解析用户输入的工具，可以通过扫描用户输入的字符串，生成Query对象，以下是一个代码示例：
	 * 
	 * @throws Exception
	 */
	@Test
